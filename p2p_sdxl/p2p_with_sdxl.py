...
from null_inversion import SDXLDDIMPipeline
...
g_cpu = torch.Generator().manual_seed(999)


task_config = '../augmentation-pipeline/' + 'configs/Entire_Inpainting.yaml'
# Load Task Config
...
class Prompt2PromptPipeline(StableDiffusionXLPipeline):
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            half_added_cond_kwargs = {"text_embeds": half_add_text_embeds, "time_ids": half_add_time_ids}
            ...
            for i, t in enumerate(timesteps):
                ...
                if inpaint:
                    ...
                    for ind_ in range(1):
                        ...
                        noise_pred = new_sd.unet(latent_model_input, t.to(TYPE_), 
                                                encoder_hidden_states=half_prompt_embeds,
                                                added_cond_kwargs=half_added_cond_kwargs, ).sample
                        ...
                        if needs_upcasting:
                            ...
                            image_pred = image_pred.to(dtype=torch.float16)
                        ...
                        print("inpaint_error", inpaint_error.item(), "meas_error", meas_error.item())
                ...
                with torch.no_grad():
                    ...
                    if i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                        ...
                        if callback is not None and i % callback_steps == 0:
                            ...
                            callback(step_idx, t, prev_latents)
...
from processing.save_image import save_synthetic_samples_txt
import random
import matplotlib.pyplot as plt

var_prompt = {'person' : [" person wearing fuzzy clothes", " person wearing red clothes", " person wearing striped clothes", " person wearing dotted clothes", " person wearing yellow clothes", " person wearing green clothes", " person wearing orange clothes"],
              'boat' : [" rusty boat", " dirty boat", " orange boat", " green boat", " shiny boat", " red boat", " blue boat"],
              'dog' : [" fuzzy dog", " muddy dog", " brown dog", " spotted dog"],
              'cat' : [" fuzzy cat", " striped cat", " brown cat", " spotted cat"],
              'cow' : [" fuzzy cow", " black cow", " all brown cow", " spotted cow"],
              'sheep' : [" fuzzy sheep", " stiped sheep", " brown spotted sheep", " black spotted sheep"],
              'bird' : [" fuzzy bird", " striped bird", " brown spotted bird", " black spotted bird"],
              'horse' : [" fuzzy horse", " striped horse", " brown spotted horse", " black spotted horse"],
              'sofa' : [" fuzzy sofa", " red sofa", " dotted sofa"],
              'tvmonitor' : [" shiny tvmonitor", " blurry tvmonitor", " dusty tvmonitor"],
              'bottle' : [" etched bottle", " striped bottle", " red bottle", " rusty bottle"],
              'diningtable' : [" etched diningtable", " striped diningtable", " dirty diningtable"],
              'chair' : [" fuzzy chair", " striped chair", " dotted chair", " dirty diningtable"],
              'pottedplant' : [" flowering pottedplant", " dying pottedplant", " green pottedplant", " dirty pottedplant"],
              'car' : [" rusty car", " sports car", " striped car", " shiny car"],
              'bicycle' : [" dotted bicycle", " red bicycle", " striped bicycle", " rusty bicycle"],
              'aeroplane' : [" dotted aeroplane", " red aeroplane", " striped aeroplane", " rusty aeroplane"],
              'train' : [" dotted train", " red train", " rusty train", " orange train"],
              'motorbike' : [" dotted motorbike", " red motorbike", " striped motorbike", " rusty motorbike"],
              'bus' : [" rusty bus", " red bus", " orange bus", " lined bus", " dotted bus"] }

style_prompt = ['drawing', 'watercolor', 'colorpencil', 'painting', 'pointillism', 'mosaic', 'pastel']
...
def prompt_variation(obj, samples, label_path):
    prompt = ["a realistic photo of"] * samples
    obj = parse_txt_objects(label_path, decoder)
    for i, key in enumerate(obj):
        if i > 0:
            prompt = [p + " and" for p in prompt]
        prompt = [p + " " + str(obj[key]) for p in prompt] 
        new = random.sample(var_prompt[key], samples)
...
for image in images[0:]:
    ...
    for i, key in enumerate(obj):
        if i > 0:
            prompt += " and"
        prompt += " " + str(obj[key]) + " " + key
    
    # Process Image
    org_image = img.to(device)
    org_image = org_image.permute(2, 0, 1).unsqueeze(0) / 255
    org_image = org_image/org_image.max()
    org_image = F.interpolate(org_image, (1024, 1024))

    mask_gen = mask_generator(
        file_type = data_config['ext'],
    ...
    for prompt in prompts[1:]:
        n_cross_replace_words = n_cross_replace_words | set(prompt.split())
...
"""
    ...
    image_rec = torch.tensor(np.array(image_rec).transpose(2, 0, 1)).cpu()
    image_rec = torch.clamp((image_rec/image_rec.max()), min=0.0, max=1.0)
    rec_meas = torch.unsqueeze(operator.forward(image_rec.to(device), mask=mask[0].to(device)), 0)
    org_image = (org_image + 1) / 2
    y = operator.forward(org_image, mask=mask)
    y_n = noiser(y)

    mse = torch.nn.functional.mse_loss(rec_meas, y)
    print(f"MSE Loss: {mse}")
    mses.append(mse.item())

    rec_fill = torch.unsqueeze(operator.forward(image_rec.to(device), mask=torch.abs(1 - mask[0]).to(device)), 0)
    # rec_fill = match_color_intensity(rec_fill[0].cpu(), y[0].cpu())
    # rec_fill = torch.tensor(rec_fill).permute(2, 0, 1).unsqueeze(0)

    inpainted = y.to(device) + rec_fill.to(device)
    inpainted_img = inpainted.cpu().numpy().transpose(0,2,3,1)[0]*255
    Image.fromarray(inpainted_img.astype(np.uint8)).save(os.path.join(sample_path, corruption_type, "glued",f"{base_count:05}.png"))

    org_image = org_image.cpu().numpy().transpose(0,2,3,1)[0]*255
    Image.fromarray(org_image.astype(np.uint8)).save(os.path.join(sample_path, corruption_type, "img",f"{base_count:05}.png"))

    y_n = (y_n + 1) / 2
    y_n = y_n.cpu().numpy().transpose(0,2,3,1)[0]*255
    Image.fromarray(y_n.astype(np.uint8)).save(os.path.join(sample_path, corruption_type, "input",f"{base_count:05}.png"))

    print("AVG CLIP SCORE:", np.mean(clip_scores))
    print("AVG MSE LOSS:", np.mean(mses))
